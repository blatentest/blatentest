...
/*------------------------------------------------*/
		...
		if (!('زورو' in chat)) chat.زورو = false 
  ...
  user.commandCount = 1
	        ...
	        let hl = _prefix 
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `*لم يعد معك جواهر قم بشراء جواهر مثال .شراء 1*`, m)
        ...
        if (!m.fromMem && m.text.match(/(بلاتين|بوته|بوت|تست|blaten| bot|BLATEN)/gi)) {
        let emot = pickRandom(["🎃", "❤", "💎", "⚡", "❓", "😎", "🙌", "⭐", "⁦👨🏻‍💻⁩", "🔥"])
...
export async function callUpdate(callUpdate) {
    ...
    //await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
     let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Yosef Ragab 👑;;;\nFN:Yosef Ragab 👑\nORG:Yosef Ragab 👑\nTITLE:\nitem1.TEL;waid=256765617305:+256765617305 \nitem1.X-ABLabel:Yosef Ragab👑\nX-WA-BIZ-◤SAKURA - Bot ◢:[❗] كلم الرقم ده - المطور.\nX-WA-BIZ-NAME:Yosef Ragab 👑\nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: 'Yosef Ragab 👑', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = (type, m, conn) => {
    ...
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] تنبيه - إشعار', body: 'ᴛʜᴇ KOMAJ - ʙᴏᴛ', thumbnail: imagen1, sourceUrl: 'https://github.com/BrunoSobrino/TheMystic-Bot-MD' }}}}, aa)
